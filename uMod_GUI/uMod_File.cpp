...
namespace
{
const unsigned char kTpfPassword[] = {
...
int uMod_File::CreateTpfPackage(const wxString &output_path, const wxArrayString &files, const wxString &author, wxString &error)
{
  ...
  HZIP zip_handle = CreateZip(temp_path.wc_str(), reinterpret_cast<const char*>(kTpfPassword));
  if (zip_handle == NULL)
  {
    error = Language->Error_TpfCreate;
    return -1;
...
int uMod_File::AddZip( AddTextureClass &tex, bool add, bool tpf)
{
  ...
  if (tpf)
  {
    ...
    return AddContent( reinterpret_cast<const char*>(pw), tex, add);
  }
  else
  {
